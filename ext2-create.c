...
#define max16 32767 /*max value for signed int 16*/
...
void write_superblock(int fd) {
	...
	// TODO finish the superblock number setting DONE
	...
	superblock.s_max_mnt_count     = max16; /* Make this unlimited */
...
void write_block_group_descriptor_table(int fd) {
	...
	// TODO It's all yours DONE
	// TODO finish the block group descriptor number setting DONE
...
void write_block_bitmap(int fd)
{
	...
	// TODO It's all yours DONE
	u8 map_value[BLOCK_SIZE]; //arr of 8 bit ints
	memset(map_value, 0xFF, NUM_BLOCKS);
	memset(map_value + 2, 0x7F, 1); //mark last bit of byte 2 as free
	memset(map_value + 3, 0, NUM_FREE_BLOCKS); //mark byte 3 onwards free
	memset(map_value + 127 , 0x80, 1); //first bit in use rest free
	memset(map_value + 128, 0xFF, NUM_BLOCKS - 128); //can't use blocks 1024-8192
...
void write_inode_bitmap(int fd)
{
	...
	memset(map_value, 0x00, NUM_INODES); //clear all bits
	memset(map_value, 0xFF, 1); //first 8 inodes used
	memset(map_value + 1, 0x1F, 1);
	memset(map_value + 2, 0x00, NUM_FREE_INODES); //clear rest of bits
	memset(map_value + 16, 0xFF, BLOCK_SIZE - NUM_INODES / 8); //mark the rest we aren't using as in use
...
void write_inode_table(int fd) {
	...
	// TODO It's all yours DONE
	// TODO finish the inode entries for the other files DONE
	//same inode structure for root, hello world, hello
	...
	struct ext2_inode hworld_inode = {0};
	hworld_inode.i_mode = EXT2_S_IFREG
	                      | EXT2_S_IRUSR
	                      | EXT2_S_IWUSR
	                      | EXT2_S_IRGRP
	                      | EXT2_S_IROTH;
	hworld_inode.i_uid = 1000;
	hworld_inode.i_size = 12;
	hworld_inode.i_atime = current_time;
	hworld_inode.i_ctime = current_time;
	hworld_inode.i_dtime = 0;
	hworld_inode.i_gid = 1000;
	hworld_inode.i_links_count = 1;
	hworld_inode.i_blocks = 2; /* These are oddly 512 blocks */
	hworld_inode.i_block[0] = HELLO_WORLD_FILE_BLOCKNO;
	write_inode(fd, HELLO_WORLD_INO, &hworld_inode);

	struct ext2_inode h_inode = {0};
	h_inode.i_mode = EXT2_S_IFLNK
	                      | EXT2_S_IRUSR
	                      | EXT2_S_IWUSR
	                      | EXT2_S_IRGRP
	                      | EXT2_S_IROTH;
	h_inode.i_uid = 1000;
	h_inode.i_size = 11;
	h_inode.i_atime = current_time;
	h_inode.i_ctime = current_time;
	h_inode.i_dtime = 0;
	h_inode.i_gid = 1000;
	h_inode.i_links_count = 1; //parent
	h_inode.i_blocks = 0; /* These are oddly 512 blocks */
	memcpy(&(h_inode.i_block), "hello-world", 11);
	write_inode(fd, HELLO_INO, &h_inode);
}
...
void write_root_dir_block(int fd)
{
	...
	/* writes dir entries ., .., hello-world, hello, lost+found, definitely repetitive can be done w loop 
	but keeping structure of lost and found example*/
	...
	struct ext2_dir_entry hworld_entry = {0};
	dir_entry_set(hworld_entry, HELLO_WORLD_INO, "hello-world");
	dir_entry_write(hworld_entry, fd);
	bytes_remaining -= hworld_entry.rec_len;

	struct ext2_dir_entry h_entry = {0};
	dir_entry_set(h_entry, HELLO_INO, "hello");
	dir_entry_write(h_entry, fd);
	bytes_remaining -= h_entry.rec_len;
